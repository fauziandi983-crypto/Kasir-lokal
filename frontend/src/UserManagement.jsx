  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px', flexWrap: 'wrap', gap: '12px' }}>
        <h2 style={{ margin: 0 }}>👥 {currentUser?.role === 'owner' ? 'Kelola Klien' : 'Kelola Kasir'}</h2>
        {currentUser?.role !== 'owner' && (
          <button className="btn btn-success" onClick={() => setShowForm(!showForm)}>
            {showForm ? '✕ Batal' : '+ Tambah Kasir Baru'}
          </button>
        )}
      </div>
      ...
      {showForm && (
        <div className="glass-panel" style={{ padding: '24px', marginBottom: '24px' }}>
          <h3 style={{ marginBottom: '16px' }}>Tambah Kasir Baru</h3>
          <form className="grid-4-cols" onSubmit={handleAddUser} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr auto', gap: '12px', alignItems: 'end' }}>
            ...
            <div>
              ...
              <select value={newUser.role} onChange={e => setNewUser({ ...newUser, role: e.target.value })} style={{ color: 'black' }} disabled>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ background: 'rgba(16, 185, 129, 0.1)' }}>
              ...
              {currentUser?.role === 'owner' && (
                <>
                  <th style={{ padding: '14px 16px', textAlign: 'left', fontWeight: 600, fontSize: '13px', color: 'var(--text-secondary)' }}>KLIEN</th>
                  <th style={{ padding: '14px 16px', textAlign: 'left', fontWeight: 600, fontSize: '13px', color: 'var(--text-secondary)' }}>TOKO</th>
                </>
              )}
          ...
          <tbody>
            {users.map(user => (
              <tr key={user.id} style={{ borderBottom: '1px solid rgba(148, 163, 184, 0.1)' }}>
                ...
                {currentUser?.role === 'owner' && (
                  <>
                    <td style={{ padding: '14px 16px' }}>{user.nama_klien || '-'}</td>
                    <td style={{ padding: '14px 16px' }}>{user.nama_toko || '-'}</td>
                  </>
                )}
            ...
            {users.length === 0 && (
              <tr>
                <td colSpan={currentUser?.role === 'owner' ? 6 : 4} style={{ padding: '24px', textAlign: 'center', color: 'var(--text-secondary)' }}>
